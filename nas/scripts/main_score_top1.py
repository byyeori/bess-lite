"""
Weighted Score Top1 모델 학습 스크립트 (main.py와 동일한 변인통제)
Trial ID: 20251123112951_6869
Config: hidden_dim=24, depthwise_kernel=3, dilated_layers=3, dilation=[1], ema_decay=0.80, head_dropout=0.00
Resources: FLOPs=127,638, Params=3,699
"""
...
def main():
    ...
    print(f"🚀 Weighted Score Top1 모델 학습 시작")
    ...
    for epoch in range(EPOCHS):
        ...
        if val_metric < best_val:
            best_val = val_metric
            best_epoch = epoch
            torch.save(model.state_dict(), CKPT_DIR / 'score_top1_model.pth')
    ...
    model.load_state_dict(torch.load(CKPT_DIR / 'score_top1_model.pth', map_location=device))
