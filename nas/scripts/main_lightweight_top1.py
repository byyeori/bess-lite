"""
경량화 Top1 모델 학습 스크립트 (main.py와 동일한 변인통제)
Trial ID: 20251123111431_8195
Config: hidden_dim=20, depthwise_kernel=3, dilated_layers=1, dilation=[1], ema_decay=0.95, head_dropout=0.10
Resources: FLOPs=91,206, Params=2,619
"""
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        if val_metric < best_val:
            best_val = val_metric
            best_epoch = epoch
            torch.save(model.state_dict(), CKPT_DIR / 'lightweight_top1_model.pth')
    ...
    model.load_state_dict(torch.load(CKPT_DIR / 'lightweight_top1_model.pth', map_location=device))
